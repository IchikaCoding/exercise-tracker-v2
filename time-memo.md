...
- fetch でよく使うもの
  ...
  - 自分で throw して 404 も「失敗扱い」にする

- searchDate() の中で起きるエラーを外側で拾いたいなら、その非同期結果を「待って」から try/catch する必要がある。
- async で定義した関数は Promise のチケットだけを返す

  - それを try の中に書いてもエラーがキャッチできない！
